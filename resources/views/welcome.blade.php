...
<section class="container pt-5 my-5 pb-lg-4">
  <div class="row pt-0 pt-md-2 pt-lg-0">
    ...
    <div class="col-xl-8 col-lg-10 order-3 mt-lg-n5">
      <form class="form-group d-block" action="{{ route('propiedades.venta', 'filter') }}" method="POST">
        @csrf
        <div class="row g-0 ms-sm-n2">
          <div class="col-md-8 d-sm-flex align-items-center">
            <div class="dropdown w-sm-50 border-end-sm" data-bs-toggle="select">
              <button class="btn btn-link ps-2 ps-sm-3" type="button" data-bs-toggle="dropdown"><i class="fi-home me-2"></i><span class="dropdown-toggle-label">En venta</span></button>
              <input type="hidden">
            </div>
            <hr class="d-sm-none my-2">
            <div class="dropdown w-sm-50 border-end-sm" data-bs-toggle="select">
              <button class="btn btn-link dropdown-toggle ps-2 ps-sm-3" type="button" data-bs-toggle="dropdown"><i class="fi-map-pin me-2"></i><span class="dropdown-toggle-label">Ubicaci&oacute;n</span></button>
              <input type="hidden" name="estado">
              <ul class="dropdown-menu">
                @foreach ($estados as $estado)
                <li><a class="dropdown-item" href="#"><span class="dropdown-item-label">{{ $estado->estado }}</span></a></li>  
                @endforeach
              </ul>
            </div>
            <hr class="d-sm-none my-2">
            <div class="dropdown w-sm-50 border-end-md" data-bs-toggle="select">
              <button class="btn btn-link dropdown-toggle ps-2 ps-sm-3" type="button" data-bs-toggle="dropdown"><i class="fi-list me-2"></i><span class="dropdown-toggle-label">Tipo de Propiedad</span></button>
              <input type="hidden" name="tipo_vivienda">
              <ul class="dropdown-menu">
                @foreach ($tipos as $tipo)
                <li><a class="dropdown-item" href="#"><span class="dropdown-item-label">{{$tipo->tipo_vivienda}}</span></a></li>  
                @endforeach
              </ul>
            </div>
          </div>
          ...
          <div class="col-md-4 d-sm-flex align-items-center pt-4 pt-md-0">
            <div class="d-flex align-items-center w-100 pt-2 pb-4 py-sm-0 ps-2 ps-sm-3"><i class="fi-cash fs-lg text-muted me-2"></i><span class="text-muted">Precio</span>
              <div class="range-slider pe-0 pe-sm-3" data-start-min="{{ $maxPrice->precio_sugerido / 2 }}" data-min="{{ $minPrice->precio_sugerido }}" data-max="{{ $maxPrice->precio_sugerido }}" data-step="50000">
                <div class="range-slider-ui"></div>
                <input class="form-control range-slider-value-min" name="maxPrice" type="hidden">
              </div>
            </div>
            <button class="btn btn-icon btn-primary px-3 w-100 w-sm-auto flex-shrink-0" type="submit"><i class="fi-search"></i><span class="d-sm-none d-inline-block ms-2">Buscar</span></button>
          </div>
        </div>
      </form>
    </div>
      ...
      <div class="col-lg-6 offset-lg-1 col-md-7 col-12">
          ...
          <div class="steps steps-vertical">
              ...
              <div class="step active">
                  ...
                  <div class="step-label ms-4">
                      <h3 class="h5 mb-2 pb-1">Te prestamos el enganche inicial o la diferencia</h3>
                      <p class="mb-0">¡Despreocúpate del enganche! Aptoclick te facilita el acceso a la vivienda de tus sueños con un préstamo hasta el 20% del valor avalúo de la casa.</p>
                  </div>
              </div>
              <div class="step active">
                  <div class="step-progress"><span class="step-number">6</span></div>
                  <div class="step-label ms-4">
